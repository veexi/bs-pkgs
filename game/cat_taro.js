let weather = new Map();
weather.set('sunny', '晴天');
weather.set('rainy', '下雨');
weather.set('cloudy', '多云');
weather.set('snowy', '雪天');
weather.set('windy', '有风');

// 随机选择一个天气
let weatherKeys = Array.from(weather.keys());
let randomWeatherKey = weatherKeys[Math.floor(Math.random() * weatherKeys.length)];
let randomWeather = weather.get(randomWeatherKey);

/*
later 10000 {
    drrr.print("/me 唔~,今天的天气是 " + randomWeather + " ! 正常开业!")
}
*/
...
event join (user) => {
   ...
   if (guests.includes(user)){
     drrr.print("/me 欢迎光临 " + new_user +"，有什么需要的吗？|输入\"来\u200B一杯 饮品\"来获取随机饮品，或：\"来\u200B一杯 自定义饮品名\"来获取想要的饮品。|")
   }else{
      guests.push(user);
      drrr.print("/me 欢迎光临 " + new_user +"，有什么需要的吗？|输入\"来\u200B一杯 饮品\"来获取随机饮品，或：\"来\u200B一杯 自定义饮品名\"来获取想要的饮品。|输入\"猫\u200BTaro\"可以进行单张塔罗牌占卜。|");
    }
}
...
startTimer=()=>{
  setTimeout(() => {
    drrr.print("/me "+new Date().toLocaleTimeString());  // 输出当前时间（不包含日期）
    startTimer();  // 重新启动定时器，设置新的随机间隔
