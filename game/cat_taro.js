...
let drinks =  [
              ...
              {
                "name":"椰林飘香",
                "method": "白朗姆酒45ml、鲜榨菠萝汁90ml、椰子奶油30ml，在调酒壶中加冰摇合，滤入盛满冰块的Tiki杯中，用菠萝角和樱桃挂在杯沿装饰。将制作完成的『椰林飘香(Pino Colada)』"
              }
            ]
...
let splitReply = (message) => {
    ...
    while (remaining.length > 100) {
        ...
        let lastCode = remaining.charCodeAt(end - 1);
...
let sendChunkedLow = (message) => {
    let parts = splitReply(message);
    let i = 0;
    while (i < parts.length) {
        let prefix = parts.length > 1 ? "(" + (i + 1) + "/" + parts.length + ") " : "";
        drrr.low(prefix + parts[i]);
        i++;
    }
};

let sendChunkedDm = (user, message) => {
    let parts = splitReply(message);
    let i = 0;
    while (i < parts.length) {
        let prefix = parts.length > 1 ? "(" + (i + 1) + "/" + parts.length + ") " : "";
        drrr.dm(user, prefix + parts[i]);
        i++;
    }
};
...
event join (user) => {
   ...
   if (guests.includes(user)){
     sendChunkedLow("欢迎光临 " + new_user +"，这次需要喝什么呢？还是来一次占卜？");
   }else{
      guests.push(user);
      sendChunkedLow("欢迎光临 " + new_user +"，有什么需要的吗？|输入「来一杯 饮品／飲品」来获取随机推荐，或「来一杯 忘忧」自定义饮品。|输入「牌阵列表／牌陣列表」查看猫塔罗牌阵。|");
    }
}
    ...
    if(command.indexOf("来一杯") === 0 || command.indexOf("来一杯") === 1){
        if(command.includes("来一杯")){
            ...
            let sendDrink = (method, targetUser) => {
                ...
                if(parts.length === 2) {
                    sendChunkedLow(parts[0] + "。");
                    sendChunkedLow("将制作完成的" + parts[1] + "递给 " + targetUser);
                } else {
                    sendChunkedLow(method + "递给 " + targetUser);
                }
            };
            ...
            if(drink !== ("饮品")){
                ...
                if(foundDrink) {
                    ...
                    sendChunkedLow("给 " + new_user1 + " 递上 " + drink);
                }
            }else{
...
event[msg, me, dm, low](user, cont: "(来一份|來一份)\\s+\\S") => {
    ...
    if(command.indexOf("来一份") === 0 || command.indexOf("来一份") === 1){
        if(command.includes("来一份")){
            ...
            if(food === ("甜品")){
                sendChunkedLow("给 " + new_user1 + " 递上 " +random_dessert);
            }else if(food === ("食品")){
                sendChunkedLow("给 " + new_user1 + " 递上 " +random_food);
            }else{
                sendChunkedLow("给 " + new_user1 + " 递上 " +food);
            }
        }
    }
}
...
event[msg, me, dm ,low](user, cont: "(圣火与猫|聖火與貓)", tc, url, req) => {
    ...
    if(command.indexOf("圣火与猫") === 1 || command.indexOf("圣火与猫") === 0){
        if(command.includes("圣火与猫")){
            if (req.type == "dm"){
                sendChunkedDm(user,"所有的猫都崇拜热量。我们寻找有阳光的地点，在热乎乎的石头上伸展开身体，跟同伴温暖地蜷缩在一起。对于圣火与猫宗教而言，比起单纯的温暖，火焰意味着更多。是内在闪耀的光亮，是激励人心的高贵的猫族精神在追求卓越。充满激情的恋人们在选择伴侣时，理由正当，圣火与猫是技巧超高的战士。勇敢的历险和形象化，它们热爱黑夜也热爱点亮黑夜的火焰。好奇心和勇气通常引领它们进入充满挑战的境地，因为它们都是无畏的。火族猫的起源随着时间的流逝和神话的传承已变得模糊，在英勇和征服故事中夸张地把火族猫描述成伟大的纺纱工。它们称自己是太阳女神 Sekhment 的后代，它们作为圣殿守护者和猫教皇的传承由来已久的故事是真的。它们是宗族中最高尚的，火族猫寻求能滋养它们灵魂的神秘猎物。");
            }else{
                sendChunkedLow("所有的猫都崇拜热量。我们寻找有阳光的地点，在热乎乎的石头上伸展开身体，跟同伴温暖地蜷缩在一起。对于圣火与猫宗教而言，比起单纯的温暖，火焰意味着更多。是内在闪耀的光亮，是激励人心的高贵的猫族精神在追求卓越。充满激情的恋人们在选择伴侣时，理由正当，圣火与猫是技巧超高的战士。勇敢的历险和形象化，它们热爱黑夜也热爱点亮黑夜的火焰。好奇心和勇气通常引领它们进入充满挑战的境地，因为它们都是无畏的。火族猫的起源随着时间的流逝和神话的传承已变得模糊，在英勇和征服故事中夸张地把火族猫描述成伟大的纺纱工。它们称自己是太阳女神 Sekhment 的后代，它们作为圣殿守护者和猫教皇的传承由来已久的故事是真的。它们是宗族中最高尚的，火族猫寻求能滋养它们灵魂的神秘猎物。");
            }
        }
    }
}
...
event[msg, me, dm, low](user, cont: "(海洋与猫|海洋與貓)", tc, url, req) => {
    ...
    if(command.indexOf("海洋与猫") === 1 || command.indexOf("海洋与猫") === 0){
        if(command.includes("海洋与猫")){
            if (req.type == "dm"){
                sendChunkedDm(user,"猫与水是老朋友了，当然我们可以选择友谊的方式。有的会游泳，从高空跳水。有的仅在玩耍时向喷泉伸出自己漂亮的小爪子，或只是单纯地看着闪闪发光的喷泉。水是水族猫的母亲，恋人，家和幸福。他们的心向往海边由月亮引起的潮汐运动，河流动时创造的音乐，阳光照耀下的湖光粼粼波光。水族猫是所有猫中最超然的。他们知晓你所想，你可以相信他们并分享你的秘密。水族猫情绪化，喜怒无常，深沉和明智。宗族神话说水族猫来自一个大岛，现已沉入海底，一些猫变成 MERCAT 仍然生活在海底。当它们凝视时，流露出只有他们宗族可以看到的那种梦幻般的眼神，也许它们听到的就是 MERCAT 的歌声。");
            }else{
                sendChunkedLow("猫与水是老朋友了，当然我们可以选择友谊的方式。有的会游泳，从高空跳水。有的仅在玩耍时向喷泉伸出自己漂亮的小爪子，或只是单纯地看着闪闪发光的喷泉。水是水族猫的母亲，恋人，家和幸福。他们的心向往海边由月亮引起的潮汐运动，河流动时创造的音乐，阳光照耀下的湖光粼粼波光。水族猫是所有猫中最超然的。他们知晓你所想，你可以相信他们并分享你的秘密。水族猫情绪化，喜怒无常，深沉和明智。宗族神话说水族猫来自一个大岛，现已沉入海底，一些猫变成 MERCAT 仍然生活在海底。当它们凝视时，流露出只有他们宗族可以看到的那种梦幻般的眼神，也许它们听到的就是 MERCAT 的歌声。");
            }
        }
    }
}
...
event[msg, me, dm, low](user, cont: "(天空与猫|天空與貓)", tc, url, req) => {
    ...
    if(command.indexOf("天空与猫") === 1 || command.indexOf("天空与猫") === 0){
        if(command.includes("天空与猫")){
            if (req.type == "dm"){
                sendChunkedDm(user,"就跟他们的齿爪一样，风族猫的性格也是非常的犀利。他们不信任任何人，甚至他们自己的族人。他们一年四季露天生活，他们怀疑墙的保护是限制和诱惑。这一弱点使他们防范心强，紧张，好斗。风族猫是机智无情的敌人。如果他们不能找到真正的猎物，他们就会知道麻烦。那些不合群的就待在宗群领地的边缘处，其他的拉帮结伙成抢劫团伙欺负那些大摇大摆通过他们领地的比较温和的猫。打架是家常便事，在交配季节尤其激烈。风族的猫只在意他们的爪子，对公共意识毫不关心。最明智的是学会控制自己的脾气，充分利用自己的智慧。其他的生命则是短暂充满暴力的。风族起源于那些流放猫，和平和地生活而被迫离开的。经历数代后，等级就逐渐形成，他们的国王和王后都是通过战争取得的，通过世袭传承固定下来的。目前没有人敢挑战他们。");
            }else{
                sendChunkedLow("就跟他们的齿爪一样，风族猫的性格也是非常的犀利。他们不信任任何人，甚至他们自己的族人。他们一年四季露天生活，他们怀疑墙的保护是限制和诱惑。这一弱点使他们防范心强，紧张，好斗。风族猫是机智无情的敌人。如果他们不能找到真正的猎物，他们就会知道麻烦。那些不合群的就待在宗群领地的边缘处，其他的拉帮结伙成抢劫团伙欺负那些大摇大摆通过他们领地的比较温和的猫。打架是家常便事，在交配季节尤其激烈。风族的猫只在意他们的爪子，对公共意识毫不关心。最明智的是学会控制自己的脾气，充分利用自己的智慧。其他的生命则是短暂充满暴力的。风族起源于那些流放猫，和平和地生活而被迫离开的。经历数代后，等级就逐渐形成，他们的国王和王后都是通过战争取得的，通过世袭传承固定下来的。目前没有人敢挑战他们。");
            }
        }
    }
}
...
event[msg, me, dm, low](user, cont: "(大地与猫|大地與貓)", tc, url, req) => {
    ...
    if(command.indexOf("大地与猫") === 1 || command.indexOf("大地与猫") === 0){
        if(command.includes("大地与猫")){
            if (req.type == "dm"){
                sendChunkedDm(user,"土族称他们是猫族的第一支。且不论真假，它讲出了最重要的事情：历史，稳定，安全。土族猫深深地隶属于他们的领地。他们一旦找到能给他们和平感觉的好地方，他们就不会再动。从内心深处安于一隅，欣赏自己所拥有的和美食。自然的意趣吸引着他们，他们与其他动物的互动多于另外三个宗族（除了猎物）。尽管若需要，土族猫愿意为自己的所有而战，他们更可能通过共同的努力等待对手退出或把他们排挤出局。他们对自己的宗族感到自豪并保存了宗族的传说并传承着。虽然宗族排首位，土族猫本性慷慨，对贫穷的人也欢迎，关心所有人的福祉。和善，安静，喜欢舒适，土族猫在他们的世界里自在的生活着。");
            }else{
                sendChunkedLow("土族称他们是猫族的第一支。且不论真假，它讲出了最重要的事情：历史，稳定，安全。土族猫深深地隶属于他们的领地。他们一旦找到能给他们和平感觉的好地方，他们就不会再动。从内心深处安于一隅，欣赏自己所拥有的和美食。自然的意趣吸引着他们，他们与其他动物的互动多于另外三个宗族（除了猎物）。尽管若需要，土族猫愿意为自己的所有而战，他们更可能通过共同的努力等待对手退出或把他们排挤出局。他们对自己的宗族感到自豪并保存了宗族的传说并传承着。虽然宗族排首位，土族猫本性慷慨，对贫穷的人也欢迎，关心所有人的福祉。和善，安静，喜欢舒适，土族猫在他们的世界里自在的生活着。");
            }
        }
    }
}
